...
public class SimpleSchoolBookService implements BookService<SchoolBook> {
    ...
    public Author findAuthorByBookName(String name) {
        SchoolBook[] bookArray = schoolBookBookRepository.findByName(name);
        Author author = null;
        if(bookArray.length!=0){
                author = authorService.findByFullName(bookArray[0].getAuthorName(),bookArray[0].getAuthorLastName());
        }
        return author;
    }
}
